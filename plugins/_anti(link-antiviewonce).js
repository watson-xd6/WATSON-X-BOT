...
handler.before = async function (m, { isAdmin, isBotAdmin }) {
    ...
    if (chat.antiLink && isGroupLink && !isAdmin && !m.isBaileys && m.isGroup) {
        ...
        if (!isBotAdmin) m.reply(` *「 ANTILINK 」* ${isAdmin ? "Admins have full control, you know! :'v" : `\n\nLink group detected, action will be taken! What do you want the bot to do next? ${global.namebot} Not an admin, so I can't kick.!`}`)
        if (isBotAdmin) {
