...
let handler = async (m, { conn, text, participants, usedPrefix, command }) => {
	if (!text) throw `_*Enter number*!_\nContoh:\n\n${usedPrefix + command} ${global.owner[0]}`
    ...
    if (anu[0].attrs.error == 408) conn.reply(m.chat, `*Cannot add* @${anu[0].attrs.jid.split('@')[0]}!\n*Maybe* @${anu[0].attrs.jid.split('@')[0]} *Just left this group or got kicked?*`, m)
    for (const user of participant[0].content.filter(item => item.attrs.error == 403)) {
    	const jid = user.attrs.jid
    	const content = getBinaryNodeChild(user, 'add_request')
    	const invite_code = content.attrs.code
    	const invite_code_exp = content.attrs.expiration
    	const txt = `*Inviting* @${jid.split('@')[0]} *Using an invite*...`
