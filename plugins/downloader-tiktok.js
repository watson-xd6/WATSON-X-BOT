    ...
    if (
      ...
      if (videoData.images?.length) {
        for (let i = 0; i < videoData.images.length; i++) {
          const caption = i === 0 ? `Here’s the image, dude. ${i + 1}\n\n${info}` : `Here’s the image, dude. ${i + 1}`
          await conn.sendFile(m.chat, videoData.images[i], `image${i + 1}.jpg`, caption, m)
        }
      } else throw "No image available."
    } else {
      if (videoURL || videoURLWatermark)
        await conn.sendFile(m.chat, videoURL, isDouyin ? "douyin.mp4" : "tiktok.mp4", `Here’s the video, bro/sis.\n\n${info}`, m)
