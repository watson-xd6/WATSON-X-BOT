...
let handler = async (m, { conn, command }) => {
    try {
        ...
        conn.sendFile(m.chat, pp, "nih bang.png", 'Done....', m, { jpegThumbnail: await (await fetch(pp)).buffer() })
    } catch {
        let pp = await conn.profilePictureUrl(m.sender, 'image').catch(e => './src/avatar_contact.png')
        conn.sendFile(m.chat, pp, 'ppsad.png', "Done....", m, { jpegThumbnail: await (await fetch(pp)).buffer() })
    }
}
