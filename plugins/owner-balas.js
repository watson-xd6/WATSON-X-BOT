...
let handler = async (m, { conn, text, usedPrefix, command }) => {
   ...
   // if (jid == m.sender) throw 'tidak bisa mengirim pesan memfess ke diri sendiri.'
    ...
    try {
    	let id = + new Date
        let txt = `Hai @${data.jid.split('@')[0]}, You received a message from: *Owner*\nPesan: \n${pesan}`.trim();
...
handler.help = ['balas'].map(v => v + ' <number|message>')
handler.tags = ['owner']
